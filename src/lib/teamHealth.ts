...
// 신뢰가 없으면 건강한 갈등이 불가능하고, 갈등이 없으면 결정 이행이 안 된다.
...
export const HEALTH_LAYERS: HealthLayer[] = [
  ...
  {
    key: 'commitment', level: 3, name: '결정 이행',
    dysfunction: '각자 다른 이해',
    symptom: '같은 회의를 했는데 며칠 뒤 "그게 그런 결정이었어?"가 나온다. 반대 의견은 회의 후 1:1로만 돈다.',
    intervention: '회의 직후 결정 사항·담당·기한을 메시지 채널에 한 줄로 남긴다. 이견이 있으면 그 스레드에서.',
  },
  {
...
export function interpretHealth(scores: LayerScore[]): HealthInterpretation | null {
  ...
  if (broken) {
    ...
    const analysis: Record<HealthLayerKey, string> = {
      trust: '팀원들이 약점이나 실수를 드러내기 어려운 환경입니다. 겉으로는 협력적으로 보여도 실제로는 각자 방어적으로 움직이고 있을 가능성이 있습니다. 이 상태에서는 그 위의 갈등·결정 이행·책임 모두 표면적으로만 작동합니다.',
      conflict: prevHealthy
        ? '신뢰는 쌓였지만 진짜 이견을 꺼내는 데까지는 아직입니다. 회의에서는 고개를 끄덕이다가 회의 밖에서 "사실 나는..."이 나오는 패턴이 있을 수 있습니다.'
        : '신뢰가 낮으니 건강한 이견 자체가 나오기 어렵습니다. 틀릴 것 같거나 관계가 불편해질까봐 조용히 넘어가는 패턴이 반복됩니다.',
      commitment: prevHealthy
        ? '회의에서 이견은 나오지만 결론이 모두에게 같은 방식으로 닫히지 않고 있습니다. 며칠 뒤 "그게 그런 결정이었어?"가 나오거나, 각자 다른 버전으로 실행하는 일이 생길 수 있습니다.'
        : '이견 없이 만들어진 결정은 각자 자기 방식으로 해석하기 쉽습니다. 누가 무엇을 얼마나 해야 하는지 합의된 그림이 없는 상태입니다.',
      accountability: prevHealthy
        ? '결정은 공유됐지만 서로의 기준을 요구하는 데까지는 아직입니다. 기준에 못 미치는 걸 봐도 말을 꺼내기 어색하고, 피드백은 팀장만 하게 됩니다.'
        : '각자 다른 이해로 실행하고 있으니 서로에게 기준을 요구하기도 어렵습니다. 지적은 불편한 일, 팀장이 할 일로 남겨집니다.',
      results: prevHealthy
  ...
  if (broken) {
    ...
    if (affected.length > 0) {
      ...
      const particle = (word: string) => { const c = word.charCodeAt(word.length - 1); return c >= 0xAC00 && (c - 0xAC00) % 28 !== 0 ? '이' : '가' }
      chainPara = `${names}도 기준선 아래입니다. ${broken.layer.name}${particle(broken.layer.name)} 잡히지 않은 상태에서 이 층들만 따로 개선하려 해도 효과가 잘 나지 않습니다 — Lencioni 모델의 핵심이 바로 이 누적 구조입니다.`
    }
  }
