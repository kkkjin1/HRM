...
// 신뢰가 없으면 건강한 갈등이 불가능하고, 갈등이 없으면 명확한 결정이 안 나온다.
...
export const HEALTH_LAYERS: HealthLayer[] = [
  ...
  {
    key: 'commitment', level: 3, name: '명확한 결정',
    dysfunction: '명확한 결정의 결여',
...
export function interpretHealth(scores: LayerScore[]): HealthInterpretation | null {
  ...
  if (broken) {
    ...
    const analysis: Record<HealthLayerKey, string> = {
      trust: '팀원들이 약점이나 실수를 드러내기 어려운 환경입니다. 겉으로는 협력적으로 보여도 실제로는 각자 방어적으로 움직이고 있을 가능성이 있습니다. 이 상태에서는 그 위의 갈등, 명확한 결정, 책임 모두 표면적으로만 작동합니다.',
      ...
      accountability: prevHealthy
        ? '결정은 명확히 공유됐지만 서로의 기준을 요구하는 단계가 아직 안 됐습니다. 기준 미달을 봐도 짚어주기가 어색하고, 피드백은 팀장 몫이 되어버립니다.'
