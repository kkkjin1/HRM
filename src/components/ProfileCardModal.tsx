...
export default function ProfileCardModal({ memberId, onClose }: Props) {
  ...
  function nameOf(id: string) { return members.find(m => m.id === id)?.name ?? '알 수 없음' }
      ...
      >
        ...
        <div className="relative pt-9 pb-5 px-6 text-center overflow-hidden" style={{ background: `linear-gradient(180deg, ${palette.bg} 0%, #ffffff 100%)` }}>
          ...
          <div className="flex items-center justify-center gap-1.5 mt-1 flex-wrap">
            {member.position && <span className="text-[11.5px] px-2 py-0.5 rounded-full bg-white/70 text-[#3A4249] border border-[#EEF0F2]">{member.position}</span>}
              ...
              <div>
                ...
                <form onSubmit={e => { e.preventDefault(); addNote() }} className="flex gap-1.5">
                  <input value={noteDraft} onChange={e => setNoteDraft(e.target.value)}
                    placeholder={noteKind === 'praise' ? `${member.name}님 칭찬 한마디` : noteKind === 'warning' ? `${member.name}님한테 (장난) 경고장` : `${member.name} 덕분에 가능했던 것`}
