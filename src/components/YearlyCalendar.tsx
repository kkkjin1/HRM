...
export default function YearlyCalendar() {
  ...
  function renderMonthCard(month: number) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        <p className="text-[12.5px] font-semibold text-[#7A8491]">상반기 (1~6월)</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {FIRST_HALF.map(renderMonthCard)}
        </div>
      </div>

      <div className="space-y-2">
        <p className="text-[12.5px] font-semibold text-[#7A8491]">하반기 (7~12월)</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {SECOND_HALF.map(renderMonthCard)}
        </div>
      </div>
    </div>
  )
