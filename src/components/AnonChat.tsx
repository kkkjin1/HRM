...
// 어느 탭을 보고 있든 좌측에 항상 떠 있는 패널이라 h-full로 부모(aside)의 높이를 그대로 채운다.
...
export default function AnonChat() {
  ...
  return (
    <div className="bg-white border border-[#E8E8E4] rounded-2xl p-4 h-full flex flex-col min-h-0">
      <div className="flex-shrink-0 mb-3">
        <p className="text-[12px] text-[#9C9C96]">🕵️ 오늘의 익명 채팅</p>
        <p className="text-[10.5px] text-[#B0B0AA] mt-0.5">여기 남긴 이야기는 오늘 하루만 보여요.</p>
      </div>

      <div ref={listRef} className="flex-1 min-h-0 space-y-2 overflow-y-auto mb-3">
        {!loaded ? (
          ...
          messages.map(m => (
            <div key={m.id} className="bg-[#F7F7F5] rounded-xl px-3 py-2 max-w-[90%]">
      ...
      >
        <input
          ...
          disabled={!me}
          className="flex-1 min-w-0 text-[13px] border border-[#E8E8E4] rounded-lg px-3 py-2 focus:outline-none focus:border-[#5B54C4] disabled:opacity-50"
        />
        <button
          type="submit"
